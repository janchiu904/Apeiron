...
public class SFRenderer : MonoBehaviour {
	...
	private void RenderLightMap(RenderTexture target, bool shadows, List<SFLight> lights, Rect viewBounds, Color ambient){
		...
		foreach(var light in lights){
			...
			if(shadows && light._shadowLayers != 0){
				...
				var mesh = light._GetShadowMesh(lightPolys, _minLightPenetration);
				if(mesh != null){
...
//		Debug.Log ("OnPreRender Camera "+Camera.current.cameraType+ " is in scene view in Unity: " + Application.unityVersion);
		...
		RenderLightMap(_lightMap, false, lights, viewBounds, ambient);
		
		if(_shadows){
			_ShadowMap = GetTexture(_shadowMapScale);
			RenderLightMap(_ShadowMap, true, lights, viewBounds, ambient);
		}
		GL.PopMatrix();
