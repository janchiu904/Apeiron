		...
		Pass {
			...
			CGPROGRAM
				...
				void VShader(VertexInput IN, out FragmentInput OUT){
					// Unpack input.
					float radius = max(1e-5, IN.properties[0]);
					...
					OUT.penumbras = float4(penumbraA, penumbraB);
					...
					OUT.edges.y *= 2.0;
					OUT.edges.z = dot(segmentNormal, projected.xy - segmentPosition*projected.w);

					// World space values.
					OUT.worldPosition = float4(mul(unity_ObjectToWorld, projected).xy/lightPenetration, 0.0, clipPosition.w);
					float2 segA = mul(unity_ObjectToWorld, float4(segmentA, 0.0, 1.0)).xy;
					float2 segB = mul(unity_ObjectToWorld, float4(segmentB, 0.0, 1.0)).xy;
